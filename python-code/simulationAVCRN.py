...
class Simulation:
    ...
    def runSimulations(self) -> float:
        self.setWeekSchedule()
        
        # Variabelen voor de uiteindelijke gemiddelden
        total_OV = 0
        ...
        # Print header alleen als we meerdere replicaties binnen runSimulations doen
        if self.R > 1:
            print("r \t Normal_OV \t Anti_OV \t Combined_OV")

        for r in range(self.R):
            # ================= 1. NORMAL RUN =================
            # We gebruiken de huidige random state (gezet door de main)
            # Maar voor AV moeten we binnen de replicatie wel kunnen resetten
            start_seed = random.randint(0, 1000000) 
            
            random.seed(start_seed) 
            self.resetSystem()
            self.generatePatients(is_antithetic=False) 
            ...
            # ================= 2. ANTITHETIC RUN =================
            random.seed(start_seed) # Reset naar exact dezelfde seed voor AV
            self.resetSystem()
            self.generatePatients(is_antithetic=True) 
            self.runOneSimulation()
        ...
        # Bereken het gemiddelde over de replicaties in deze aanroep
        final_avg_OV = total_OV / self.R
        
        # CRUCIAAL: Stuur de waarde terug naar de main blok!
        return final_avg_OV
