...
class Simulation:
    ...
    def generatePatients(self, is_antithetic=False) -> None:
        """
        Gegenereerde patiënten voor zowel Normal als Antithetic runs.
        is_antithetic: True voor de antithetische run (gebruikt 1-U), 
                       False voor de normale run (gebruikt U).
        """
        counter = 0
        self.patients = [] # Reset de lijst voor de huidige run

        for w in range(self.W):
            for d in range(self.D):
                
                # --- ELECTIVE PATIENTS ---
                if d < self.D - 1: # Niet op zondag
                    current_time = 8.0
                    while True:
                        # Gebruik de nieuwe helper die U of 1-U kiest op basis van de vlag
                        # Deze vervangt de dubbele aanroep uit de oude code
                        inter_arrival = self.get_exponential_sync(self.lambdaElective, is_antithetic)
                        current_time += inter_arrival * (17 - 8)
                        
                        if current_time >= 17:
                            break
                        
                        # Voor volledige variantiereductie moeten ook deze antithetisch zijn
                        ...
                        self.patients.append(Patient(counter, 1, 0, w, d, current_time, tardiness, noShow, duration))
                        counter += 1

                # --- URGENT PATIENTS ---
                lmbd = self.lambdaUrgent[0]
                end_time = 17
                if d == 3 or d == 5: # Donderdag/Zaterdag halve dagen
                    lmbd = self.lambdaUrgent[1]
                    end_time = 12
                
                current_time = 8.0
                while True:
                    inter_arrival = self.get_exponential_sync(lmbd, is_antithetic)
                    current_time += inter_arrival * (end_time - 8)
                    ...
                    # noShow en tardiness zijn 0 voor urgent
                    self.patients.append(Patient(counter, 2, scanType, w, d, current_time, 0, False, duration))
                    counter += 1

    def get_exponential_sync(self, lambda_val, is_antithetic):
        """Helper om synchronisatie te garanderen"""
        u = random.random()
        if u == 0: u = 0.0001
        
        # Als we in de antithetische run zitten, spiegelen we het getal
        if is_antithetic:
            u = 1 - u
            
        return -math.log(u) / lambda_val
    ...
    def runSimulations(self) -> None:
        ...
        # Variabelen voor de uiteindelijke gemiddelden over alle replicaties
        total_OV = 0
        total_electiveAppWT = 0
        total_urgentScanWT = 0
        
        # We maken een Excel aan om de resultaten per run op te slaan
        from openpyxl import Workbook
        wb = Workbook()
        ws_normal = wb.active
        ...
        headers = ["Replication", "Elective_App_WT", "Urgent_Scan_WT", "Objective_Value"]
        ws_normal.append(headers)
        ws_at.append(headers)

        print("r \t Normal_OV \t Anti_OV \t Combined_OV")

        for r in range(self.R):
            # ================= 1. NORMAL RUN =================
            random.seed(r) # Cruciaal: zet de stroom op startpunt 'r'
            self.resetSystem()
            self.generatePatients(is_antithetic=False) # Gebruikt U
            self.runOneSimulation()
            
            norm_elApp = self.avgElectiveAppWT
            norm_urScan = self.avgUrgentScanWt
            norm_OV = (norm_elApp * self.weightEl) + (norm_urScan * self.weightUr)
            
            ws_normal.append([r + 1, norm_elApp, norm_urScan, norm_OV])

            # ================= 2. ANTITHETIC RUN =================
            random.seed(r) # Cruciaal: zet de stroom EXACT terug op hetzelfde punt 'r'
            self.resetSystem()
            self.generatePatients(is_antithetic=True) # Gebruikt 1-U[cite: 5]
            self.runOneSimulation()
            
            anti_elApp = self.avgElectiveAppWT
            anti_urScan = self.avgUrgentScanWt
            anti_OV = (anti_elApp * self.weightEl) + (anti_urScan * self.weightUr)
            
            ws_at.append([r + 1, anti_elApp, anti_urScan, anti_OV])

            # ================= 3. COMBINEER (Antithetic Pair) =================
            # De schatter is het gemiddelde van het paar[cite: 5]
            combined_OV = (norm_OV + anti_OV) / 2
            
            total_OV += combined_OV
            total_electiveAppWT += (norm_elApp + anti_elApp) / 2
            total_urgentScanWT += (norm_urScan + anti_urScan) / 2

            print(f"{r+1} \t {norm_OV:.2f} \t\t {anti_OV:.2f} \t\t {combined_OV:.2f}")

        # Opslaan van het bestand
        output_file = f"Antithetic_Results_Rule{self.rule}.xlsx"
        wb.save(output_file)

        print("-" * 60)
        print(f"EINDGEMIDDELDE (n={self.R} paren):")
        print(f"Objective Value: {total_OV/self.R:.4f}")
        print(f"Elective App WT: {total_electiveAppWT/self.R:.2f}")
        print(f"Urgent Scan WT:  {total_urgentScanWT/self.R:.2f}")

if __name__ == "__main__":
    sim = Simulation(r"C:\Users\marleen\Desktop\smaproject2026\input-S1-14.txt", 500, 30, 1)
    sim.runSimulations()
